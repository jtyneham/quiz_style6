...
import { createInkAmbient } from "./core/ink-ambient.js";
...
void createInkAmbient(document.getElementById("homeInkStage"));
let current = "home";
...
function showScreen(name, { updateHistory } = {}) {
  Object.values(screens).forEach((screen) => screen.classList.remove("active", "ink-enter"));
  screens[name].classList.add("active", "ink-enter");
  window.setTimeout(() => screens[name].classList.remove("ink-enter"), 700);
  current = name;
...
async function openTile(tile) {
  ...
  if (!target) return;
  haptic(18);
  document.body.classList.remove("ink-swipe");
  void document.body.offsetWidth;
  document.body.classList.add("ink-swipe");
  await new Promise((resolve) => window.setTimeout(resolve, 285));
  await navigate(target);
  window.setTimeout(() => document.body.classList.remove("ink-swipe"), 340);
}
